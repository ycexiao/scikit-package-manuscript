...
def test_copy_journal_template_files_find_template(
    user_filesystem_with_repo, existing_templates, template
):
...
def test_copy_journal_template_files_find_template_bad(
    ...
    with pytest.raises(
        NotADirectoryError, match=r"Cannot find the provided journal_tamplate: "
    ) as exc_info:
        cwd_path = copy_journal_template_files(template, cwd_path)
...
def test_copy_journal_template_files_copy_files(
    user_filesystem_with_repo, template_files
):
    home_path, skm_repo_path, cwd_path = user_filesystem_with_repo
    os.environ["HOME"] = str(home_path)
